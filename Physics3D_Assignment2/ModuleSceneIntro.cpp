...
#define PLATFORM vec3(10,0.5f,10)
	...
	//safe
	CreateCube(PLATFORM, vec3(-43, 112, 38.3), Red, 90, axis_z);
	//START_PLATFORM
	...
	CreateCube(vec3(4, 0.5, 15), vec3(-85, 78.7, 38.3), Orange, 18, axis_z);
	...
	//Big fall
	CreateCube(vec3(30, 0.5, 100), vec3(-120, 50, 88), White, 30, axis_x);
	
	CreateCube(vec3(15, 2, 60), vec3(-120, 16.5, 155), Pink, 20, axis_x);
	
	//Road
	CreateCube(vec3(22, 2, 65), vec3(-120, 0, 225), Pink);
	CreateCube(vec3(230, 2, 22 ), vec3(-20, 0, 247), Pink);
	CreateCube(vec3(55, 2, 22), vec3(270, 0, 247), Pink);
	CreateCube(vec3(22,2,200), vec3(290,0,150), Pink);
	CreateCube(vec3(50, 2, 22), vec3(290, 0, 40), Pink);
	CreateCube(vec3(50, 2, 22), vec3(260, 0, 40), Pink, -10, axis_z);
	...
	//Devil Square
	CreateCube(vec3(30, 0.5, 25), vec3(108, 4, 247), White, 20, axis_z);
	CreateCube(vec3(30, 0.5, 25), vec3(234, 4, 247), White, -20, axis_z);
	...
	trap.color = Red;
	trap_01 = App->physics->AddBody(trap,App->scene_intro, false, 0);


	trap2.SetPos(114, 19, 260);
	trap2.color = Red;
	trap_02 = App->physics->AddBody(trap2, App->scene_intro, false, 50);
	...
	trap_2.color = Red;
	trap_01_2 = App->physics->AddBody(trap_2, App->scene_intro, false, 0);


	trap2_2.SetPos(1000, 72, 1000);
	trap2_2.color = Red;
	trap_02_2 = App->physics->AddBody(trap2_2, App->scene_intro, false ,50);
...
void ModuleSceneIntro::CreateCube(vec3 dim, vec3 pos, Color color, float angle, vec3 u, float mass)
{
	...
	App->physics->AddBody(cube, App->scene_intro, false, mass);
